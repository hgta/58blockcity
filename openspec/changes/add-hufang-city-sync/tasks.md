...
- [x] 4.4 掉榜城市排名清零：CitySyncer::apply() 事务内将不在榜单中的旧排名城市 rank 置 0（stat.demoted）——修复「红河 0 区块上榜」残留排名问题（红河经查是从默认视图按旧 rank=198 进榜，与排序视图无关）。清零范围最初误用 `rank > 0`，会把管理员手动维护的 200+/300+ 排名一并误清，已收窄为 `rank BETWEEN 1 AND 200`（官方榜语义区间，手动排名不受影响）；top200city.php 默认视图同步收窄为 `WHERE rank BETWEEN 1 AND 200`（防官方榜匹配不足 200 时手动排名被 LIMIT 补入）；排序视图的 0 值过滤经确认数据现实（官方 200 城全在库且全部非 0，恰好满榜）为冗余，已移除保持 SQL 语义纯粹
