...
- [ ] **#20** 替换商品/店铺/城市/圈子/NFT 列表中的旧链接为伪静态链接
- [x] **#21** 在商品/店铺/城市/圈子详情页增加旧 URL 到规范 URL 的 301 检测跳转
- [x] **#22** 为商品详情页注入 `Product` JSON-LD 结构化数据
- [ ] **#23** 为店铺详情页注入 `Store` JSON-LD 结构化数据
- [ ] **#24** 为首页注入 `Organization` JSON-LD 结构化数据
- [ ] **#25** 为店铺详情页、NFT 详情页、圈子详情页增加面包屑导航
- [ ] **#26** 为商品详情页面包屑增加 JSON-LD `BreadcrumbList`
- [ ] **#27** 商品/店铺列表页分页增加 `rel="next"` / `rel="prev"`（可选）
- [ ] **#28** 图片 `alt` 属性补全与统一（商品/店铺/圈子/NFT）
- [ ] **#29** 页面内链网格：商品详情页“相关商品”、店铺页“店内商品”、城市页“热门内容”

## 收尾

- [ ] **#30** 本地测试：检查 TDK、canonical、sitemap、伪静态、重定向状态码
- [ ] **#31** 提交 commit 并推送到远程仓库
