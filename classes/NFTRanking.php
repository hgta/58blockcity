...
class NFTRanking {
    ...
    public function getTopCitiesByPurchaseRequests($limit = 10, $period = 'all') {
        ...
        $sql = "SELECT 
                ...
                WHERE pr.status = 'active'  -- 使用status字段替代expires_at
                {$periodCondition}
                GROUP BY c.id
                ORDER BY request_count DESC
                LIMIT ?";
        
        // 方案B：如果只有created_at字段，统计所有未过期的（假设30天有效期）
        /* $sql = "SELECT 
                    c.id,
                    c.name,
                    c.image,
                ...
                FROM cities c
                JOIN nft_purchase_requests pr ON c.id = pr.city_id
                WHERE pr.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)  -- 30天内创建的求购
                {$periodCondition}
                GROUP BY c.id
                ORDER BY request_count DESC
                LIMIT ?"; */
    ...
    public function getTopNftsByPurchaseRequests($limit = 10, $period = 'all') {
        ...
        $sql = "SELECT 
                ...
                WHERE pr.status = 'active'  -- 或者使用其他条件
                {$periodCondition}
                GROUP BY n.id
                ORDER BY request_count DESC
                LIMIT ?";
     ...
     * 获取用户在求购榜的排名
     */
    private function getUserRankInPurchaseRequests($userId, $period) {
        $periodCondition = $this->getPeriodCondition($period, 'pr.created_at');
        
        $sql = "SELECT position FROM (
                    SELECT 
                        u.id,
                        ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT pr.id) DESC) as position
                    FROM users u
                    JOIN nft_purchase_requests pr ON u.id = pr.user_id
     ...
     * 获取用户在成交榜的排名
     */
    private function getUserRankInTransactions($userId, $period) {
        $periodCondition = $this->getPeriodCondition($period, 't.created_at');
