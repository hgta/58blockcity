...
// 获取已存在的NFT信息和标签
$nftInfoById = [];  // 以id为键
$nftInfoByImage = []; // 以base_image为键
$allTags = [];
try {
    // 获取NFT信息
    $stmt = $pdo->query("SELECT id, code, base_image, avatar_id FROM nft_avatars");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $nftInfoById[$row['id']] = $row;
        $nftInfoByImage[$row['base_image']] = &$nftInfoById[$row['id']];
    }
    ...
    // 获取NFT的标签
    if (!empty($nftInfoById)) {
        $nftIds = array_keys($nftInfoById);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if (isset($nftInfoById[$row['nft_avatar_id']])) {
                if (!isset($nftInfoById[$row['nft_avatar_id']]['tags'])) {
                    $nftInfoById[$row['nft_avatar_id']]['tags'] = [];
                }
                $nftInfoById[$row['nft_avatar_id']]['tags'][] = $row;
            }
        }
    }
} catch (PDOException $e) {
    ...
    <div class="gallery">
        <?php foreach ($images as $image): ?>
            <?php 
				...
				$currentNftInfo = $nftInfoByImage[$image] ?? null;
