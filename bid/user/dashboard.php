...
.dash-card { background: var(--surface); border: 1px solid var(--line); border-radius: 14px; padding: 24px; box-shadow: var(--shadow); margin-bottom: 16px; }
.dash-head { display: flex; align-items: center; gap: 16px; margin-bottom: 20px; }
.dash-avatar { width: 64px; height: 64px; border-radius: 50%; overflow: hidden; background: var(--surface-2); flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
.dash-avatar img { width: 100%; height: 100%; object-fit: cover; }
.dash-name { font-size: 20px; font-weight: 700; color: var(--text); }
.dash-sub { font-size: 13px; color: var(--muted); }
.dash-stats { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }
.dash-stat { text-align: center; padding: 16px; background: var(--surface-2); border-radius: 10px; }
.dash-stat .num { font-size: 22px; font-weight: 800; color: var(--brand); font-variant-numeric: tabular-nums; }
.dash-stat .lbl { font-size: 12px; color: var(--muted); margin-top: 4px; }
.dash-links { display: flex; flex-direction: column; gap: 0; }
.dash-link { display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid var(--line); text-decoration: none; color: var(--text); font-size: 14px; }
.dash-link:hover { background: var(--brand-dim); }
.dash-link:last-child { border-bottom: none; }
.dash-link .arrow { color: var(--muted); }
</style>
