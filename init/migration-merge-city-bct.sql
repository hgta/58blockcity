...
--
-- 本脚本（可重复执行）为 cities 增加三列并回填既有价格：
...
CREATE PROCEDURE `MergeCityBctColumns`()
BEGIN
    -- 1) cities 增加 BCT 单价列（默认 0.10，与旧 city_bct 建表默认一致）
    IF NOT EXISTS (
        ...
        ALTER TABLE `cities`
            ADD COLUMN `bct_base_price` decimal(10,2) NOT NULL DEFAULT '0.10'
            COMMENT 'BCT 基础单价（人气值市场底价）';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE `cities`
            ADD COLUMN `bct_current_price` decimal(10,2) NOT NULL DEFAULT '0.10'
            COMMENT 'BCT 当前单价（人气值市场实时价）';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE `cities`
            ADD COLUMN `bct_price_updated` datetime DEFAULT NULL
            COMMENT 'BCT 单价最近更新时间';
    END IF;
