...
CREATE TABLE IF NOT EXISTS `bct_price_history` (
  ...
  -- 显式对齐 cities.name 的排序规则（utf8mb4_unicode_ci），避免比较时报 #1267
...
CREATE TABLE IF NOT EXISTS `blocks` (
  ...
  `description` text,
  `display_type` enum('none','image','text') NOT NULL DEFAULT 'none' COMMENT '区块皮肤: none=默认, image=图片, text=文字',
  `display_image` varchar(255) DEFAULT NULL COMMENT '图片模式下的图片路径',
  `display_text` varchar(50) DEFAULT NULL COMMENT '文字模式下的文字内容',
  `display_color` enum('red','green','blue') DEFAULT NULL COMMENT '文字模式背景色',
...
--
-- 表的结构 `city_profiles` 真实城市资料表(统一城市门户)
--
...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  `link_xiaohongshu` varchar(500) DEFAULT NULL COMMENT '小红书售卖链接',
  `link_taobao` varchar(500) DEFAULT NULL COMMENT '淘宝售卖链接',
  `link_douyin` varchar(500) DEFAULT NULL COMMENT '抖音售卖链接',
  `link_kuaishou` varchar(500) DEFAULT NULL COMMENT '快手售卖链接',
  `link_jd` varchar(500) DEFAULT NULL COMMENT '京东售卖链接',
  `link_pdd` varchar(500) DEFAULT NULL COMMENT '拼多多售卖链接',
  `link_wechat_shop` varchar(500) DEFAULT NULL COMMENT '微信小店售卖链接',
...
CREATE TABLE IF NOT EXISTS `models` (
  ...
  `video_url` varchar(500) DEFAULT NULL COMMENT '模特视频地址（直链或外链，管理员维护）',
  `video_cover` varchar(255) DEFAULT NULL COMMENT '视频封面图（Hero 兜底）',
  ...
  `intro` varchar(255) DEFAULT NULL COMMENT '一句话个人简介',
  ...
  `drama_count` int(11) NOT NULL DEFAULT 0 COMMENT '参演短剧数（冗余）',
  `status` enum('active','inactive') DEFAULT 'active',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
-- --------------------------------------------------------
-- 短剧（Drama）：模特参演的短剧作品，支撑「模特 × 短剧」双向导流
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- 模特 / 作者合作申请（与 init/migrate-applications.sql 保持一致）
-- --------------------------------------------------------
